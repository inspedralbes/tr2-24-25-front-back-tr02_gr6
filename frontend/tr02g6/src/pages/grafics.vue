...
async function fetchSociograma() {
  try {
    ...
    const charts = [
      {
        category: "popular",
        svg: popularSvg.value,
        filterFn: (d) => d.popular_SN === "X",
      },
      {
        category: "controvertit",
        svg: controvertitSvg.value,
        filterFn: (d) => d.controvertit_SN === "X",
      },
      { category: "normal", svg: normalSvg.value, filterFn: (d) => d.normal_SN === "X" },
      {
        category: "rebutjat",
        svg: rebutjatSvg.value,
        filterFn: (d) => d.rebutjat_SN === "X",
      },
      {
        category: "ignorat",
        svg: ignoratSvg.value,
        filterFn: (d) => d.ignorat_SN === "X",
      },
      {
        category: "agresivitat",
        svg: agresivitatSvg.value,
        filterFn: (d) => d.totalAgressivitat != null,
        sizeValueFn: (d) => d.totalAgressivitat,
      },
      {
        category: "victimitzacio",
        svg: vicitmitzacioSvg.value,
        filterFn: (d) => d.totalVictimitzacio != null,
        sizeValueFn: (d) => d.totalVictimitzacio,
      },
    ];
...
function initD3Chart(category, svgElement, filterFn, sizeValueFn = null) {
  ...
  const sizeScale = sizeValueFn
    ? d3.scaleLinear().domain(d3.extent(data, sizeValueFn)).range([5, 30])
    : null;
  ...
  const simulation = d3
    ...
    .force(
      "collision",
      d3.forceCollide().radius((d) => (sizeScale ? sizeScale(sizeValueFn(d)) : 10))
    );

  let links = data
    .map((source, i) => data.slice(i + 1).map((target) => ({ source, target })))
    .flat();
