...
app.get("/classes", async (req, res) => {
  ...
  app.get("/classes/:course_code", async (req, res) => {
    ...
    try {
      connection = await mysql.createConnection(dbConfig);
      console.log(`Fetching classes for course_code: ${courseCode}`); // Agregado
      ...
      console.log("Query result:", rows); // Agregado
      res.json(rows);
    } catch (err) {
  ...
  app.post("/classes", async (req, res) => {
    ...
    if (!classe || !codi_random || !id_curs) {
      return res.status(400).send("Faltan parámetros: classe y/o codi_random");
    }
