...
app.delete("/alumnes", async (req, res) => {
    ...
    if (!id_alumne || isNaN(id_alumne)) {
        return res.status(400).json({ missatge: "Falta el paràmetre id_alumne o el valor es inválido" });
    }
    ...
    try {
        ...
        res.status(500).json({ missatge: "Error en eliminar alumne", error: err.message });
    }
});
